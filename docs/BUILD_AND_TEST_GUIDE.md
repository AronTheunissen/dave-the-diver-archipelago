...
3. Copy the required DLLs to `client/lib/`:
   ...
   **From `Dave the Diver/BepInEx/core/`** — copy and **rename** as follows:

   | File in `BepInEx/core/`               | Copy to `client/lib/` as          |
   |---------------------------------------|-----------------------------------|
   | `BepInEx.Core.dll`                    | `BepInEx.dll`                     |
   | `0Harmony.dll`                        | `HarmonyX.dll`                    |
   | `BepInEx.Unity.IL2CPP.dll`            | `BepInEx.Unity.IL2CPP.dll` (no rename needed) |
   | `Il2CppInterop.Runtime.dll`           | `Il2CppInterop.Runtime.dll` (no rename needed) |

   **From `Dave the Diver/BepInEx/interop/`** — copy into `client/lib/` (NOT the interop subfolder) and rename:

   | File in `BepInEx/interop/`    | Copy to `client/lib/` as  |
   |-------------------------------|---------------------------|
   | `il2cppmscorlib.dll`          | `Il2Cppcorlib.dll`        |

   **From `Dave the Diver/BepInEx/interop/`** — copy into `client/lib/interop/` (create that folder if needed):
   - `Assembly-CSharp.dll`
   - `UnityEngine.dll`
   - `UnityEngine.CoreModule.dll`

   > ⚠️ The `interop/` folder is **generated by BepInEx on first launch** — if it doesn't exist yet, go back to Step 1 and launch the game once.

   > ✅ **Archipelago.MultiClient.Net** does NOT need to be copied manually — it is pulled in automatically via NuGet when you run `dotnet build`. There is no need to find it in a `Managed/` folder.
